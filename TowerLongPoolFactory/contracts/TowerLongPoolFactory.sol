...
import "./TowerLongPool.sol";

contract TowerLongPoolFactory is Ownable {
    mapping(address => address) public towerPools; // token => TowerLongPool
    mapping(address => address) public tokenForTowerPool; // TowerLongPool => token
        ...
        returns (address towerPool)
    {
        ...
        bytes memory bytecode = type(TowerLongPool).creationCode;
        assembly {
            towerPool := create2(0, add(bytecode, 32), mload(bytecode), _stake)
        }
        TowerLongPool(towerPool)._initialize(
            _stake,
    ...
    function claimRewards(
        ...
        for (uint256 i = 0; i < _towerPools.length; i++) {
            TowerLongPool(_towerPools[i]).getReward(msg.sender, _tokens[i]);
        }
    }
    ...
    function whitelistTowerPoolRewards(
        ...
        for (uint i; i < len; ++i) {
            TowerLongPool(_towerPools[i]).whitelistNotifiedRewards(_rewards[i]);
        }
    }
    ...
    function removeTowerPoolRewards(
        ...
        uint len = _towerPools.length;
        for (uint i; i < len; ++i) {
            TowerLongPool(_towerPools[i]).removeRewardWhitelist(_rewards[i]);
        }
    }
    ...
    struct TowerPoolInfo {
      ...
      TowerLongPool.RewardInfo[] rewardInfoList;
      uint256 totalSupply;
    ...
    function getInfoForAllTowerPools(address account) external view returns (TowerPoolInfo[] memory towerPoolInfoList) {
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            TowerLongPool towerPool = TowerLongPool(towerPoolAddress);
